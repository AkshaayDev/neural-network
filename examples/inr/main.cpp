...
void createImage(std::string name) {
	...
	(void)std::system(("node ./txt2png.js " + name).c_str());
}
...
// First, the image is converted to a .hpp file with a flattened array of grayscale values using `img2hpp.js`
// Then, the data is loaded into this script which creates a network or loads a pre-existing one from `./nn.dat`
// After training, the network is saved to `./nn.dat` and the output image is created with `./txt2png.js`
int main() {
